...
**Dashboard rendering (Phase 2 of TRACK-275):** The runner-mode session viewer in `src/ui/core.html` consumes every event through a part registry (`SESSION_PART_RENDERERS`) — one renderer per event kind. Tool-use and edit rows are expandable cards: clicking shows pretty-printed args, the tool's full output (≤64KB, syntax-highlighted via highlight.js loaded lazily on first expand), and unified diffs (rendered via diff2html, also lazy-loaded). Both libraries are vendored locally under `src/ui/vendor/` (diff2html 3.4.51 slim bundle, highlight.js 11.10.0) so the dashboard is fully self-sufficient — no third-party CDN at runtime. Diffs over 500 lines are gated behind a confirmation button to avoid main-thread stalls. Bash output gets a terminal-styled header with a Copy-command button. Assistant text renders as markdown via the shared `renderMarkdown()` helper. Multi-line steering composer auto-grows up to 8 lines; smart auto-scroll only sticks to the bottom when the user is already there (otherwise a `↓ N new` pill appears). The same renderer runs for both the live SSE feed and the audit-log transcript replay.
...
- **Merge Candidates panel (TRACK-289).** Topbar button "Merge Candidates" opens a pull-only side panel listing pairs from `GET /embeddings/merge-candidates` ranked by similarity. Each row shows both keys, titles, and a similarity score; threshold is adjustable inline. Per-row actions: **Preview** (toggles a side-by-side description preview), **Merge…** (prefills the existing merge modal — defaults target to the higher seq-number item, user can flip), **Dismiss** (POSTs to `/embeddings/tombstones` so the pair stops appearing). No badges, no auto-popup. Refreshes itself after a merge submit.
- **Topics panel (TRACK-289).** Topbar button "Topics" opens a pull-only side panel listing clusters from `GET /embeddings/topics`, sorted by size. Each row shows an inline-editable label (click to rename → `PATCH /embeddings/clusters/:id/label`), member count, representative item key, and a brief titles preview. Clicking a cluster drills into a detail view of all members (clickable cards that open the item) with a **Group these as a new item** button that calls `POST /items/group` to turn the cluster into a real parent_of group. No badges, no auto-popup. The API excludes done/cancelled members and also suppresses clusters whose live members are already a single connected component in the existing link graph (any relation) — those items are already linked/grouped, so showing them as a new topic adds no value.
